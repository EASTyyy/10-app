(function () {
    ...
    angular.module('10Style.activity')
        .controller('ActivityTemplate3Ctrl', ActivityTemplate3Ctrl);

    ActivityTemplate3Ctrl.$inject = ["$scope", "$stateParams", "$ionicScrollDelegate", 'Activity', "ProductGroup", "ComponentParser",
    "Loading", "showBottomSlogan", "IMAGE_PARAMS", "SessionStorage", "Product", "Config", "wechatShare"];

    function ActivityTemplate3Ctrl($scope, $stateParams, $ionicScrollDelegate, Activity, ProductGroup, ComponentParser,
    Loading, showBottomSlogan, IMAGE_PARAMS, SessionStorage, Product, Config, wechatShare) {

        var SCROLLER_NAME = 'activity-3-scroller';
        ...
        function loadDataSuccess(response) {
            ...
            $scope.links = ComponentParser.execute(contents[2]);
            $scope.subtitle = ComponentParser.execute(contents[3]);

            $scope.list = ComponentParser.execute(contents[4]);
            _isAbleShare = true;
        }
